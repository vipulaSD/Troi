...
public class MainActivity extends AppCompatActivity implements EmpaStatusDelegate {
    ...
    private void makePrediction() {
        ...
        String[] labels = {"cheerful", "happy", "angry", "nervous", "sad", "neutral"};
        ...
        for (int i = 0; i < 6; i++) {
            ...
            Log.i(TAG, "makePrediction: " + labels[i] + " -> " + prediction[0][i]);
        }
        if (argmax == -1) {
            return;
        }
        String emotionLabel = labels[argmax];
        Log.i(TAG, "makePrediction: final prediction " + emotionLabel + " prob: " + max);
        emotionDistro.put(labels[argmax], emotionDistro.getOrDefault(emotionLabel, 0) + 1);
